...
Auren Transfer Agent v1.6.0 keeps the configuration contract stable and expands the Media Hub section with operational hardening controls while retaining Gateway Runtime controls and the Auren Storage production adapter fallback key. This file defines the configuration surface, precedence rules, default APIs and validation rules used by the production runtime.
...
## v1.6.0 operational hardening keys
