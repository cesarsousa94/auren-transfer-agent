# Media Hub Connector — v1.6.0

Auren Transfer Agent v1.1.0 added the Media Hub Connector Foundation. v1.6.0 keeps that contract and uses it for transfer claim/callback execution, production Auren Storage upload results and Gateway Runtime session telemetry. The Agent can now register itself as a Media Hub `edge_node`, persist the issued `node_uuid` and `node_secret`, pull node configuration and send heartbeat, metrics and events through the existing Media Hub Node Agent contract.
...
- Public gateway runtime and session telemetry are implemented in v1.6.0.
