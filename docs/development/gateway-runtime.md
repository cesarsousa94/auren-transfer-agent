# Gateway Runtime — v1.13.1

Auren Transfer Agent v1.13.1 implements the public Media Hub gateway runtime. The Agent remains an execution/data-plane component: Media Hub validates the public token, chooses the delivery policy and returns the upstream instructions.
...
v1.13.1 adds drain/backpressure admission around this gateway runtime. The gateway still delegates token validation and upstream policy to Media Hub.
