...
if [[ ! "${VERSION}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  echo "version must look like v1.6.0" >&2
  exit 1
fi
if [[ ! -x "${BINARY}" ]]; then
...
install -m 0644 "docs/deployment/linux-package-bootstrap.md" "docs/deployment/production.md" "${ROOT}/usr/share/doc/${APP_NAME}/"
